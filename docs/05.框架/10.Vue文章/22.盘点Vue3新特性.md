...
```
前面都是Vue3.0的新特性， 那相对于Vue2.x来说， 有什么变更呢？首先
...
```
### v-model 升级
在使用Vue 3 之前就了解到 `v-model` 发生了很大的变化， 使用过了之后才真正的get到这些变化， 我们先纵观一下发生了哪些变化， 然后再针对变化说一下如何使用：
...
在Vue3 中,在自定义组件上使用`v-model`,相当于传递一个`modelValue`属性， 同时出发一个`update:modelValue`事件：
